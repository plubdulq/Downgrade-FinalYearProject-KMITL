    ...
    public class DotAnimatedTexture : MonoBehaviour
    {
        ...
        void Init()
        {
            ...
            else
            {
                ...
                baseTexID = GetBaseTextureID(rendererMaterial);
            }
#if UNITY_EDITOR
        ...
        public void ForceUpdate(bool force = true)
        {
            ...
            else
            {
                ...
                if (firstFrame + newIndex != prevIndex)
                {
                    ...
                    {
                        if (baseTexID > 0){
                            _renderer.material.SetTextureOffset(baseTexID, offset); 
                        } else {
                    ...
                    else
                    {
                        ...
                        if (baseTexID > 0) {
                            rendererMaterial.SetTextureOffset(baseTexID, offset);
                        } else {
...
#endif
        private int GetBaseTextureID(Material rendererMaterial)
        {
            Shader shader = rendererMaterial.shader;
            string shaderName = shader.name;
            string[] validTextures = {
               "_EmissiveColorMap",  "HDRP",
            };
            for (int i = 0; i < validTextures.Length; i+=2)
            {
                if( (validTextures[i+1] == "") || (shaderName.IndexOf(validTextures[i+1]) > -1) ) {
                    if ((shader.FindPropertyIndex(validTextures[i]) > -1) )
                    {
                        return Shader.PropertyToID(validTextures[i]);
                    }
                    break;
                }
            }
